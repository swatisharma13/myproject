...
public class GoodyearUSQA_V7 {
	...
	@Test(dataProvider = "goodyearUSqav7-data-provider", priority =1)
	public static void login_backend(
			...
			Reporter.log(GoodyearUSQA_V7.class.getSimpleName());
	...
	Reporter.log(Environment);
			Reporter.log("Browser Version:"+caps.getVersion().toString());
			 WebDriverManager.chromedriver().setup();
			  d1 = new ChromeDriver();
			  d1.get("https://goodyear-stg.springboardats.com/login");
		...
		try{
			...
			Reporter.log(GoodyearUSQA_V7.class.getSimpleName()+"-"+UserName_BackEnd+"- Springboard Login Passed");
			}catch(Exception e){
...
@Test(priority = 7)
	...
	@DataProvider(name="goodyearUSqav7-data-provider")
	public String[][] usernameAndPasswordDataProvider() {
		return ExcelReadUtil.readExcelInto2DArray("./GoodyearUSQAv7.xlsx","goodyearUSQA",5);
	}
}