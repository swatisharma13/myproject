...
public class ClemUK_V7 {
	...
	@Test(dataProvider = "clemUKv7-data-provider", priority =1)
	...
	// public static void login_backend() 
			...
			Reporter.log(ClemUK_V7.class.getSimpleName());
	...
	Reporter.log(Environment);
			Reporter.log("Browser Version:"+caps.getVersion().toString());
			 WebDriverManager.chromedriver().setup();
			  d1 = new ChromeDriver();
		d1.get("https://clem.springboardats.co.uk/login");
		...
		try{
			...
			Reporter.log(ClemUK_V7.class.getSimpleName()+"-"+UserName_BackEnd+"- Springboard Login Passed");
			}catch(Exception e){
	 ...
	 @Test(priority = 5)
	  public static void CreateaJob() throws Exception{
		...
		d1.findElement(By.xpath("//a[contains(@href,'clientlov')]")).click();
			...
			d1.findElements(By.xpath("//a[contains(@href,'javascript:passBack')]")).get(7).click();
			d1.switchTo().window(winHandleBefore2);
			WebElement f6 = d1.findElement(By.id("raspframe"));
			d1.switchTo().frame(f6);
			Thread.sleep(500);
			//Solution Hire
			Select solutionHIre = new Select (d1.findElements((By.name("in_values"))).get(0));
			solutionHIre.selectByVisibleText("Onsite");
			//Reason
			Select recruitmentReason = new Select (d1.findElements((By.name("in_values"))).get(1));
			recruitmentReason.selectByVisibleText("New Position");
			d1.findElements(By.name("in_values")).get(2).sendKeys("1234");
			d1.findElements(By.name("in_values")).get(8).sendKeys("1234");
			d1.findElements(By.name("in_values")).get(9).sendKeys("1234");
			//Category
			...
			Thread.sleep(500);
			//Candidate Source
			Select candidateSource = new Select (d1.findElements((By.name("in_values"))).get(4));
			candidateSource.selectByVisibleText("Clemenger Group Recruitment (CGR)");
			//OnboardingPack
			String winHandleBefore3 = d1.getWindowHandle();	
			d1.findElement(By.xpath("//a[contains(@href,'javascript:docGroupLov()')]")).click();
			for(String windowHandle : d1.getWindowHandles()){
			    d1.switchTo().window(windowHandle);
			}
			d1.findElement(By.name("in_action")).click();
			Thread.sleep(500);	
			d1.findElements(By.xpath("//a[contains(@href,'javascript:passBack')]")).get(0).click();
			d1.switchTo().window(winHandleBefore3);
			WebElement f7 = d1.findElement(By.id("raspframe"));
			d1.switchTo().frame(f7);
			Thread.sleep(500);
			//Hiring Manager Details
			d1.findElements(By.name("in_values")).get(18).sendKeys("Automation User");
			d1.findElements(By.name("in_values")).get(19).sendKeys("Automation@User.com");
			d1.findElements(By.name("in_values")).get(20).sendKeys("PM");
			d1.findElements(By.name("in_values")).get(21).sendKeys("06787868688");
			d1.findElements(By.name("in_values")).get(20).sendKeys("PM");
			Select agency = new Select (d1.findElements((By.name("in_values"))).get(22));
			agency.selectByVisibleText("Clemenger BBDO");
			Select finDirector = new Select (d1.findElements((By.name("in_values"))).get(23));
			finDirector.selectByVisibleText("Clemenger BBDO Wellington");
			//Approve and Update
			d1.findElement(By.xpath("/html/body/tag_quicksearch/div[2]/form/div/div[1]/div/div/div[1]/i")).click();
		...
		public static void JobSearch() throws Exception {
			...
			Thread.sleep(1000);
			d1.findElement(By.id("JobGrid:searchForm:jobrefSuggestion_input")).sendKeys("CLM/1402372");
			Thread.sleep(1000);
	...
	public static void OrbeonForm() throws Exception{
		...
		public static void Companies() throws Exception{
			...
			// Companies
				Thread.sleep(2000);
				//d1.findElement(By.xpath("//a[contains(text(),'Companies')]")).click();
		...
		public static void delete_candidate() 
			...
			try{
				...
				Thread.sleep(2000);
				//d1.findElement(By.id("candidateSearch:j_idt1396:searchForm:j_idt1449:name")).sendKeys("Michael Kors");
				d1.findElement(By.xpath("div")
				//.sendKeys("Michael Kors");
				////*[@id$="name"]
				//#candidateSearch\:j_idt3076\:searchForm\:j_idt3129\:name
				Thread.sleep(2000);
				d1.findElement(By.id("candidateSearch:j_idt1396:searchForm:findCandidates")).click();
				Thread.sleep(3000);
				Actions actions = new Actions(d1);
				d1.findElement(By.id("candForm:candidateGrid:0:actionMenuButton")).click();
				WebElement manageCandOption = d1.findElement(By.xpath("//span[@class= 'Manage Candidate Record']"));
				actions.moveToElement(manageCandOption).perform();
			   // d1.findElement(By.xpath("//div[@id='candActionsSelector:actionSelectorMenu']/ul/li[8]/ul/li[8]/a/span")).click();
				WebElement dltCandOption = d1.findElement(By.xpath("//span[contains(text(),'Delete Candidate']"));
				dltCandOption.click();
	...
	@DataProvider(name="clemUKv7-data-provider")
	public String[][] usernameAndPasswordDataProvider() {
		return ExcelReadUtil.readExcelInto2DArray("./ClemUKv7.xlsx","clemUK",5);
	}
}
