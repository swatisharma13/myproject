...
public class Sales_V6 {
	static WebDriver d1;
	
	@Test(dataProvider = "salesV6-data-provider", priority =1)
	...
	try{
		...
		Reporter.log(Sales_V6.class.getSimpleName()+"-"+UserName_BackEnd+"- Springboard Login Passed");
		}catch(Exception e){
	...
	public static void JobTab()
		...
		//try{
			...
			Thread.sleep(1000);
			d1.findElement(By.xpath("//a[contains(text(),'JOB/1518273')]")).click();
			Thread.sleep(2000);
	...
	/*@Test(priority = 3)
	public static void Excperience_Tab()
					throws InterruptedException, FailedLoginException {
		try{
			//Candidate Experience Tab
			cr.clickSidebarMenuItem(d1,"Candidates");
			Thread.sleep(2000);
			d1.findElement(By.xpath("//input[@id='candidateSearch:searchForm:search-criteria:email']")).sendKeys("swang@peoplescout.com");
			Thread.sleep(2000);
	...
	public static void InterviewAutoCommunication()
		...
		try{
			...
			Thread.sleep(1000);
			WebElement f1 = d1.findElement(By.xpath("//div[@class='adminConsole-Tabcontent')/iframe"));
			d1.switchTo().frame(f1);
	...
	public static void Tasks()
		...
		Thread.sleep(1000);
		d1.findElement(By.xpath("//span[contains(text(),'Task')]"));
		Reporter.log("Tasks Page loaded Successfully");		
		}catch(Exception e){
	        Reporter.log("Tasks Page Is Broken");
			throw new FailedLoginException("Tasks Page Is Broken");
	    }		
		finally{	
			d1.quit();
		}
	...
	@DataProvider(name="salesV6-data-provider")
	public String[][] usernameAndPasswordDataProvider() {
		return ExcelReadUtil.readExcelInto2DArray("./salesv6.xlsx","sales",5);
	}
}
