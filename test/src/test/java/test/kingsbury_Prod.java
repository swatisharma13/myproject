...
public static String javaScript = "var evObj = document.createEvent('MouseEvents');" +
	...
	@Test(dataProvider = "kingsburyV7-data-provider", priority =1)
//	 @Test(priority =1)
	...
	// public static void login_backend() 
			...
			Reporter.log(kingsbury_Prod.class.getSimpleName());
	...
	Reporter.log(Environment);
			Reporter.log("Browser Version:"+caps.getVersion().toString());
			 WebDriverManager.chromedriver().setup();
			  d1 = new ChromeDriver();
			  d1.get("https://kingsbury.springboardats.com/jobtools/jncustomorglogin.logout?in_organid=18694");
	...
	d1.findElement(By.xpath("//input[@type='PASSWORD']")).sendKeys(Password_BackEnd);
		...
		Reporter.log(kingsbury_Prod.class.getSimpleName()+"-"+UserName_BackEnd+"- Springboard Login Passed");
			}catch(Exception e){
	...
	public static void CreateaJob() throws Exception{
		...
		//Requisition Type:
		Select ReqType = new Select (d1.findElements(By.name("in_values")).get(15));
		ReqType.selectByVisibleText("Internal/External");	
		//Hiring Manager Details
		d1.findElements(By.name("in_values")).get(16).sendKeys("Automation User");
		d1.findElements(By.name("in_values")).get(17).sendKeys("Automation@User.com");
...
public static void Interviews() throws Exception {
	...
	WebElement intelmnt = d1.findElement(By.xpath("//h5[contains(text(),'Search Interviews')]"));
...
public static void ProcessDesigner() throws Exception
	...
	@DataProvider(name="kingsburyV7-data-provider")
	public String[][] usernameAndPasswordDataProvider() {
		return ExcelReadUtil.readExcelInto2DArray("./kingsbury.xlsx","kingsbury",5);
	}
}
